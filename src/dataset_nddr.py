...
class Forecasting_Dataset_Nddr(Dataset):
    ...
    def encode_cats(self, batch):
        ...
        emb_2 = torch.Tensor(np.argmax(onehot_2, axis=1)).float()
        emb_3 = torch.Tensor(np.argmax(onehot_3, axis=1)).float()
    ...
    def __getitem__(self, idx):
        ...
        task_id = self.matches_all.index([batch[0], batch[1]])
        task = torch.Tensor(task_id)
        batch = self.encode_cats(batch)
        return [batch[0], batch[1], torch.Tensor(batch[2].astype(float))], torch.Tensor([batch[3]]), task_id #, self.matches.astype(int)
...
def get_dataloader_nddr(config, year, matches):
    train_data = Forecasting_Dataset_Nddr(path=config['DATA_PATH'], year=year, lag=config['LAG'],
                                     columns=config['COLUMNS'], matches=matches, onehot_paths=config['ONEHOT_EMBEDDERS'])
    test_data = Forecasting_Dataset_Nddr(path=config['DATA_PATH'], year=config['YEARS']['TEST'], lag=config['LAG'],
                                    columns=config['COLUMNS'], matches=matches, onehot_paths=config['ONEHOT_EMBEDDERS'])
