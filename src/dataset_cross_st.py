...
class Forecasting_Dataset_Cross(Dataset):
...
def get_dataloader_cross(config, year, matches):
    train_data = Forecasting_Dataset_Cross(path=config['DATA_PATH'], year=year, lag=config['LAG'],
                                     columns=config['COLUMNS'], matches=matches, onehot_paths=config['ONEHOT_EMBEDDERS'])
    test_data = Forecasting_Dataset_Cross(path=config['DATA_PATH'], year=config['YEARS']['TEST'], lag=config['LAG'],
                                    columns=config['COLUMNS'], matches=matches, onehot_paths=config['ONEHOT_EMBEDDERS'])
